...
import com.rjxx.taxeasy.dao.vo.JyspmxDecimal;
import com.rjxx.taxeasy.dao.vo.JyspmxDecimal2;
...
public class SeperateInvoiceUtils {
    ...
    public static List<Kpspmx> repeatSeparatePrice(Kpls kpls, List<Kpspmx> kpspmxList) throws Exception {
        ...
        for (int i = 0; i < kpspmxList.size(); i++) {
            ...
            if(kpls.getZsfs()!=null&&!kpls.getZsfs().equals("2")){
                ...
                // 商品金额不含税
                mx.setSpje(jeWithoutTax.doubleValue());
            }
    ...
    public static List<JyspmxDecimal> separatePrice(List<JyspmxDecimal> jyspmxs) throws Exception {
        ...
        // 价税分离后的list
        List<JyspmxDecimal> sepJyspmxs = new ArrayList<JyspmxDecimal>();
        for (int i = 0; i < jyspmxs.size(); i++) {
            ...
            // 商品金额不含税
            mx.setSpje(jeWithoutTax);
            // 税额
            mx.setSpse(jeTax);
            // 单价不含税
            mx.setSpdj(djWithoutTax);
            sepJyspmxs.add(mx);
        }
        return sepJyspmxs;
    }
    ...
    public static List<JyspmxDecimal2> separatePrice2(List<JyspmxDecimal2> jyspmxs) throws Exception {
        ...
        // 价税分离后的list
        List<JyspmxDecimal2> sepJyspmxs = new ArrayList<JyspmxDecimal2>();
        for (int i = 0; i < jyspmxs.size(); i++) {
    ...
    public static List<JyspmxDecimal2> splitInvoicesbhs(List<JyspmxDecimal2> jyspmxs, BigDecimal maxje, BigDecimal fpje, int mxsl, boolean qzfp,boolean spzsfp) throws Exception {
        ...
        boolean fp = false;
        // 缓存商品明细表
        List<JyspmxDecimal2> tempJyspmxs = new ArrayList<JyspmxDecimal2>();
        // 拆分发票后的list
        List<JyspmxDecimal2> splitKpspmxs = new ArrayList<JyspmxDecimal2>();
        // 汇总金额不含税
        BigDecimal zje = new BigDecimal(0);
        // 汇总金额含税
        BigDecimal zje1 = new BigDecimal(0);
        //商品明细金额总和（不含税）
        ...
        for (int i = 0; i < jyspmxs.size(); i++) {
            ...
            if ((zje.compareTo(maxje) >= 0) || (tempJyspmxs.size() == mxnum) || ((zje1.compareTo(fpje) >= 0) && qzfp)) {
                if (tempJyspmxs.size() == mxnum && zje.compareTo(maxje) <= 0 && zje1.compareTo(fpje) <= 0) {
                    ...
                    // 原商品金额
                    BigDecimal spje = jyspmx.getSpje();
                    // 原商品金额
                    BigDecimal yjshj = jyspmx.getJshj();
                    // 原商品数量
                    BigDecimal spsm = jyspmx.getSps();
                    // 原商品税额
                    BigDecimal spse = jyspmx.getSpse();
                    // 超出金额
                    BigDecimal ccje = sub(zje, maxje);
                    // 拆分金额
                    BigDecimal cfje = sub(spje, ccje);
                    /**
                     * 是否按商品整数分票
                     */
                    BigDecimal cfsm;
                    ...
                    if(spzsfp&&null!=spdj&&!"".equals(spdj)&&null!=spsm&&!"".equals(spsm)){
                        // 拆分数量
                        cfsm = div(spsm, div(spje, cfje));
                        //过度数据，计算下一次数据。
                        BigDecimal cfsm1=cfsm;
                        //向下取整
                        cfsm=new BigDecimal(Math.floor(cfsm.doubleValue()));
                        if(cfsm.compareTo(new  BigDecimal(0))!=0){
                            //多出数量的金额(不含税)
                            BigDecimal cfje1=mul(spdj,sub(cfsm1,cfsm));

                            cfje=sub(cfje,cfje1);
                            // 拆分税额
                            cfse = div(spse, div(spje, cfje)).setScale(2, BigDecimal.ROUND_HALF_UP);

                            jshj = add(cfje, cfse);
                            // 超出价税合计
                            ccjshj = sub(jyspmx.getJshj(), jshj);
                    ...
                    if (fpje.divide(new BigDecimal(1).add(jyspmx.getSpsl()), 30, BigDecimal.ROUND_HALF_UP).compareTo(maxje) > 0) {
                        ...
                        if (n > 0) {
                            ...
                            if(spzsfp&&null!=spdj&&!"".equals(spdj)&&null!=spsm&&!"".equals(spsm)){
                                // 拆分数量
                                cfsm1 = div(spsm, div(spje, maxje));
                                //过度数据，计算下一次数据。
                                BigDecimal cfsm2=cfsm1;
                                //向下取整
                                cfsm1=new BigDecimal(Math.floor(cfsm1.doubleValue()));
                                if(cfsm1.compareTo(new BigDecimal(0))!=0){
                                    //多出数量的金额(不含税)
                                    BigDecimal cfje1=mul(spdj,sub(cfsm2,cfsm1));

                                    maxje=sub(maxje,cfje1);
                                    // 拆分税额
                                    cfse1 = div(spse, div(spje, maxje)).setScale(2, BigDecimal.ROUND_HALF_UP);
                                }else{
                                    // 拆分数量
                                    cfsm1 = div(spsm, div(spje, maxje));
                                    // 拆分税额
                        ...
                        if (n > 0) {
                            ...
                            if(spzsfp&&null!=spdj&&!"".equals(spdj)&&null!=spsm&&!"".equals(spsm)){
                                ...
                                if(cfsm1.compareTo(new BigDecimal(0))!=0){
                                    //多出数量的价税合计
                                    BigDecimal cfjshj1=mul(mul(spdj,new BigDecimal(1).add(jyspmx.getSpsl())),sub(cfsm2,cfsm1));
                                    fpje=sub(fpje,cfjshj1);
                                    // 拆分税额
                                    cfse1 = div(spse, div(yjshj, fpje)).setScale(2, BigDecimal.ROUND_HALF_UP);
                                }else{
                                    // 拆分数量
                                    cfsm1 = div(spsm, div(yjshj, fpje));
                                    // 拆分税额
                    ...
                    if (fp) {
                        ...
                        if(spzsfp&&null!=spdj&&!"".equals(spdj)&&null!=spsm&&!"".equals(spsm)){
                            ...
                            if(cfsm.compareTo(new BigDecimal(0))!=0){
                                ...
                                jshj = add(cfje, cfse);
                                // 超出价税合计
                                BigDecimal ccjshj = sub(jyspmx.getJshj(), jshj);
                                ccje=add(ccje,cfje1);
                            }else{
                                // 拆分数量
                                cfsm = div(spsm, div(spje, cfje));
                        ...
                        BigDecimal spse = jyspmx.getSpse();
                        // 超出价税合计
                        BigDecimal ccjshj = sub(zje1, fpje);
                        // 拆分价税合计
                        BigDecimal cfjshj = sub(yjshj, ccjshj);
                        BigDecimal cfsm;
                        BigDecimal cfse;
                        BigDecimal cfje;
                        BigDecimal ccje;
                        ...
                        if(spzsfp&&null!=spdj&&!"".equals(spdj)&&null!=spsm&&!"".equals(spsm)){
                            ...
                            // 拆分数量
                             cfsm = div(spsm, div(yjshj, cfjshj));
                            //过度数据，计算下一次数据。
                             BigDecimal cfsm1=cfsm;
                             //向下取整
                             cfsm=new BigDecimal(Math.floor(cfsm.doubleValue()));
                             if(cfsm.compareTo(new BigDecimal(0))!=0){
                                 BigDecimal cfjshj1;
                                 //多出数量的价税合计
                                 cfjshj1 = mul(mul(spdj,new BigDecimal(1).add(jyspmx.getSpsl())),sub(cfsm1,cfsm));
                                 //把多余的价税合计去掉
                                 cfjshj=sub(cfjshj,cfjshj1);

                                 ccjshj=add(ccjshj,cfjshj1);
                                 // 拆分税额
                                 cfse = div(spse, div(yjshj, cfjshj)).setScale(2, BigDecimal.ROUND_HALF_UP);
                        ...
                        if (fpje.divide(new BigDecimal(1).add(jyspmx.getSpsl()), 30, BigDecimal.ROUND_HALF_UP).compareTo(maxje) > 0) {
                            ...
                            if (n > 0) {
                                ...
                                if(spzsfp&&null!=spdj&&!"".equals(spdj)&&null!=spsm&&!"".equals(spsm)){
                                    ...
                                    if(cfsm1.compareTo(new BigDecimal(0))!=0){
                                        //多出数量的金额(不含税)
                                        BigDecimal ccje2=mul(spdj,sub(cfsm2,cfsm1));
                                        //把最大金额减去多出数量的金额
                                        maxje=sub(maxje,ccje2);
                                        // 拆分税额
                                        cfse1 = div(spse, div(spje, maxje)).setScale(2, BigDecimal.ROUND_HALF_UP);
                                    }else{
                                        // 拆分数量
                            ...
                            if (n > 0) {
                                ...
                                if(spzsfp&&null!=spdj&&!"".equals(spdj)&&null!=spsm&&!"".equals(spsm)){
                                    ...
                                    if(cfsm1.compareTo(new BigDecimal(0))!=0){
                                        //多出数量的价税合计
                                        BigDecimal fpje1=mul(mul(spdj,new BigDecimal(1).add(jyspmx.getSpsl())),sub(cfsm2,cfsm1));
                                        //把多余的分票金额去掉
                                        fpje=sub(fpje,fpje1);
                                        // 拆分税额
                                        cfse1 = div(spse, div(yjshj, fpje)).setScale(2, BigDecimal.ROUND_HALF_UP);
    ...
    private static Map<String, BigDecimal> cfsl_byInt(BigDecimal spje, BigDecimal spdj) {
        ...
        // 取整后的金额
        BigDecimal cfje_int;
        // 取整后多出的金额 cfje = cfje_int,ccje
        BigDecimal ccje;
        // 整数数量，向下取整
        cfsm = new BigDecimal(Math.floor(cfsm.doubleValue()));
        // 整数数量对应的金额
        cfje_int = mul(spdj, cfsm);
        // 多出数量的金额(不含税)
        ccje = sub(spje, cfje_int);
