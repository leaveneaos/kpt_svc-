...
import com.rjxx.taxeasy.bizhandle.utils.InvoiceSplitParamsUtil;
import com.rjxx.taxeasy.bizhandle.utils.InvoiceSplitUtils;
...
import com.rjxx.taxeasy.dao.vo.KplsVO4;
import com.rjxx.taxeasy.dao.vo.KplsVO5;
import com.rjxx.taxeasy.dao.vo.Kpspmxvo;
import com.rjxx.time.TimeUtil;
import com.rjxx.utils.SSLClient;
...
/**
 *@ClassName FpclService
 *@Description 处理开票主类，包括分票，合并开票等
 *@Author xlm
 *@Date 2018/4/10.
 *@Version 1.0
 **/
...
public class FpclService {
    ...
    public InvoiceResponse hccl(Integer kplsh, Integer yhid, String gsdm, String hcjeStr, String xhStr) throws Exception {
        ...
        // 本次红冲金额的价税合计
        String hcjshj = df.format(hjhcje);
        ...
        for (int i = 0; i < xh.length; i++) {
            ...
            if (mxItem.getKhcje() != null) {
                ...
                if (mxItem.getSps() != null) {
                    sps = df6.format(Double.valueOf(hcje[i]) / mxItem.getJshj() * mxItem.getSps());
                }
                param1.put("sps", sps);
                ...
                // 没红冲部分的商品数量
                if (mxItem.getSps() != null) {
                    sps = df6.format(Double.valueOf(khcje) / mxItem.getJshj() * mxItem.getSps());
                }
                param1.put("sps", sps);
    ...
    public Map DzfphttpPost(String sendMes, String url, String key, String xfsh, String jylsh,int j) throws Exception {
        ...
        RequestConfig requestConfig = RequestConfig.custom().
                setSocketTimeout(1200).setConnectionRequestTimeout(1200).setConnectTimeout(1200).build();
        httpPost.setConfig(requestConfig);
        httpPost.addHeader("Content-Type", "text/xml");
        String strMessage = "";
        BufferedReader reader = null;
    ...
    public List<Object> zjkp(List<Jyxxsq> list, String kpfs) throws Exception {
        ...
        for (int j=0;j<list.size();j++) {
            ...
            //确定是否含税分票，目前只支持不含税。
            String hsbz = String.valueOf(params.get("hsbz"));
            //开票限额
            double zdje = Double.valueOf(params.get("zdje").toString());
            //分票金额
            double fpje = Double.valueOf(params.get("fpje").toString());
            //纸票分票行数
            int fphs1 = Integer.valueOf(params.get("fphs1").toString());
            //电子票分票行数
            int fphs2 = Integer.valueOf(params.get("fphs2").toString());
            //卷票票分票行数
            int fphs3 = Integer.valueOf(params.get("fphs3").toString());
            //是否强制分票
            sfqzfp = Boolean.valueOf(params.get("sfqzfp").toString());
            //是否整数分票
            boolean spzsfp = Boolean.valueOf(params.get("spzsfp").toString());
            ...
            if (hsbz.equals("1")) {
                ...
                } else if (jyxxsq.getFpzldm().equals("03")) {
                    //卷票
