...
import com.rjxx.taxeasy.domains.CorpUserBaseInfo;
...
//		HttpHelper.httpGet(url);
	...
	public static com.dingtalk.open.client.api.model.corp.CorpUserBaseInfo getUserInfo(String accessToken, String code) throws Exception{
		
		CorpUserService corpUserService = ServiceFactory.getInstance().getOpenService(CorpUserService.class);
		return corpUserService.getUserinfo(accessToken, code);
	}
	
	public static JSONObject getAgentUserInfo(String ssoToken, String code) throws Exception  {
		
		String url = Env.OAPI_HOST + "/sso/getuserinfo?" + "access_token=" + ssoToken + "&code=" + code;
		JSONObject response = HttpRequestHelper.httpGet(url);
		return response;
	}
