package com.rjxx.taxeasy.dao.vo;

public class Slcxvo {
	protected String xfmc;
	protected String fpzl;
	protected Double sl;
	protected String jzjtbz;
	protected Double zsje;
	protected Double fsje;
	protected Double hjje;
	protected Double zsse;
	protected Double fsse;
	protected Double hjse;
	public Double getSl() {
		return sl;
	}
	public void setSl(Double sl) {
		this.sl = sl;
	}
	public String getJzjtbz() {
		return jzjtbz;
	}
	public void setJzjtbz(String jzjtbz) {
		this.jzjtbz = jzjtbz;
	}
	public Double getZsje() {
		return zsje;
	}
	public void setZsje(Double zsje) {
		this.zsje = zsje;
	}
	public Double getFsje() {
		return fsje;
	}
	public void setFsje(Double fsje) {
		this.fsje = fsje;
	}
	public Double getHjje() {
		return hjje;
	}
	public void setHjje(Double hjje) {
		this.hjje = hjje;
	}
	public Double getZsse() {
		return zsse;
	}
	public void setZsse(Double zsse) {
		this.zsse = zsse;
	}
	public Double getFsse() {
		return fsse;
	}
	public void setFsse(Double fsse) {
		this.fsse = fsse;
	}
	public Double getHjse() {
		return hjse;
	}
	public void setHjse(Double hjse) {
		this.hjse = hjse;
	}
	public String getXfmc() {
		return xfmc;
	}
	public void setXfmc(String xfmc) {
		this.xfmc = xfmc;
	}
	public String getFpzl() {
		return fpzl;
	}
	public void setFpzl(String fpzl) {
		this.fpzl = fpzl;
	}
	

}
